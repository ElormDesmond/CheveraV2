...
const Portfolio = () => {
  ...
  const { data: serverProjects } = usePortfolio();
        ...
        <div className="sticky top-0 h-screen flex items-center overflow-hidden">
          <motion.div style={{ x }} className="flex gap-8 px-8">
            ...
            <div className="w-[400px] h-[600px] flex flex-col justify-center items-center text-center p-12 border-2 border-dashed border-white/10 rounded-amber">
               ...
               <h3 className="text-white text-2xl mb-4 uppercase font-display">More in the Pipeline</h3>
            </div>
          </motion.div>
        </div>
      </section>
...
const ProjectCard = ({ project, onClick }: { project: any, onClick: () => void }) => (
  ...
  >
    <img 
      src={project.thumbnail_url || project.img} 
      loading="lazy"
      decoding="async"
    ...
    <div className="absolute bottom-12 left-12 right-12 flex justify-between items-end">
       <div>
          <p className="text-amber font-black text-xs uppercase tracking-widest mb-2">{project.category || project.cat} — {project.completion_year || project.year}</p>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 z-[1001] bg-obsidian/95 backdrop-blur-2xl flex flex-col p-8 md:p-16 overflow-y-auto">
      <div className="max-w-[1800px] mx-auto w-full flex flex-col h-full">
        <div className="flex justify-between items-start mb-12">
          <div>
            <span className="text-amber font-black text-xs uppercase tracking-[0.5em] mb-2 block">{project.category || project.cat} — {project.completion_year || project.year}</span>
